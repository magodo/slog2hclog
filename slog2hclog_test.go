...
func TestLogger(t *testing.T) {
	newSimple := func(level slog.Leveler, buf *bytes.Buffer) hclog.Logger {
		...
		return slog2hclog.New(slog.New(slog.NewTextHandler(buf, &slog.HandlerOptions{
			Level: level,
	...
	t.Run("trace", func(t *testing.T) {
		...
		logger := newSimple(slog2hclog.SlogLevelTrace, &buf)
	...
	t.Run("warn", func(t *testing.T) {
		...
		assert.Equal(t, hclog.Warn, logger.GetLevel())
		assert.Equal(t, false, logger.IsTrace())
		assert.Equal(t, false, logger.IsDebug())
	...
	t.Run("error", func(t *testing.T) {
		...
		assert.Equal(t, hclog.Error, logger.GetLevel())
		assert.Equal(t, false, logger.IsTrace())
		assert.Equal(t, false, logger.IsDebug())
		assert.Equal(t, false, logger.IsInfo())
	...
	t.Run("off", func(t *testing.T) {
		var buf bytes.Buffer
		logger := newSimple(slog2hclog.SlogLevelOff, &buf)
		logger.Error("this is test", "who", "programmer", "why", "testing")
		assert.Equal(t, 0, buf.Len())

		assert.Equal(t, hclog.Off, logger.GetLevel())
		assert.Equal(t, false, logger.IsTrace())
		assert.Equal(t, false, logger.IsDebug())
		assert.Equal(t, false, logger.IsInfo())
		assert.Equal(t, false, logger.IsWarn())
	...
	t.Run("var level", func(t *testing.T) {
		...
		buf.Reset()
		lvar.Set(slog2hclog.SlogLevelTrace)
		logger.Trace("trace msg")
